    ...
    if FORCE_SUB:
        try:
          ...
          if str(user.status) == "ChatMemberStatus.BANNED":
              await message.reply_text("You are Banned. Contact -- DEATH Network")
              return 1
        except UserNotParticipant:
            link = await app.export_chat_invite_link(FORCE_SUB)
            caption = f"Join our channel to use the bot"
            await message.reply_photo(photo="https://telegra.ph/file/08ef31b89adeda339d930.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{link}")]]))
            return 1
...
async def terms(client, message):
    ...
    buttons = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("💬 Contact Now", url="https://t.me/DE4T8")],
        ]
    )
